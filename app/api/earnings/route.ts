...
async function fetchOne(symbol: string): Promise<EarningsItem> {
  ...
  if (ETF_SET.has(symbol)) {
    const item: EarningsItem = { symbol, result: null, surprisePct: null, date: null };
  ...
  try {
    ...
    // Yahoo returns history oldest → newest; take last entry
    ...
    const item: EarningsItem = { symbol, result, surprisePct, date };
    cache.set(symbol, { item, expires: Date.now() + TTL });
    return item;
  } catch {
    const item: EarningsItem = { symbol, result: null, surprisePct: null, date: null };
